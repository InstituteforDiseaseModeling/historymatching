...
param_info = pd.DataFrame(
    {"Name": ["Beta", "Gamma"], "Min": [0.0001, -2], "Max": [0.00025, -0.5]}
).set_index("Name")
...
for param_name, values in samples.iteritems():
    samples[param_name] = param_info.loc[param_name, "Min"] + values * (
...
def sim(samples, ax, reps=1):
    ...
    for idx, sample in samples.iterrows():
        for rep in range(reps):
            z = SIR(
                beta=sample["Beta"],
                gamma=np.power(10, sample["Gamma"]),
                x0=[1990, 10, 0],
            )
            T, _, P = z.sim()
            prevalence = [
                p[1] for p in P
            ]  # Analyze the simulation to get the prevalence
...
sim_results = pd.DataFrame(
    sim_results, columns=["Sample_Id", "Rep", "ObsIdx", "ObsTime", "Prevalence"]
)
data = sim_results.set_index(["Sample_Id", "Rep", "ObsIdx"])["Prevalence"].unstack(
    "ObsIdx"
)
...
g1 = GPR(
    ...
    kernel_mode="RBF",
    theta=np.array(
        [39.80141277, 0.14869185, 1.71068961, 2.0]
    ),  # np.array([4, 1, 1.4, 1.4]),
    verbose=True,
    debug=False,
)
...
if OPTIMIZE[0]:
    print("Optimizing hyperparameters for g1...")
...
if OPTIMIZE[1]:
    print("Optimizing hyperparameters for g2...")
...
print("Optimizing hyperparameters for g (GPR_MO)...")
g.optimize_hyperparameters(
    x0=[15, 15, 0.2, 0.2, 1, 0],
    bounds=(sigma2_f_bounds,)
    + (sigma2_n_bounds,)
    + b.D * (lengthscale_bounds,)
    + ((0.01, 1),)
    + ((-0.95, 0.95),),
)
...
for trial in range(N_TEST):
    ...
    for param_name, values in test.iteritems():
        test[param_name] = param_info.loc[param_name, "Min"] + values * (
            param_info.loc[param_name, "Max"] - param_info.loc[param_name, "Min"]
        )
    ...
    sim_results_test = pd.DataFrame(
        sim_results_test,
        columns=["Sample_Id", "Rep", "ObsIdx", "ObsTime", "Prevalence"],
    )

    data_test = sim_results_test.set_index(["Sample_Id", "Rep", "ObsIdx"])[
        "Prevalence"
    ].unstack("ObsIdx")
    ...
    print(f"{trial}: Evaluating g1...")
    ret1 = g1.evaluate(test)
    mean1 = ret1["Mean"]

    print(f"{trial}: Evaluating g2...")
    ret2 = g2.evaluate(test)
    mean2 = ret2["Mean"]

    print(f"{trial}: Evaluating g...")
    ret = g.evaluate(test)
    ...
    ax_test[2].plot(
        [
            mean1 - 2 * np.sqrt(ret1["Var_Latent"]),
            mean1 + 2 * np.sqrt(ret1["Var_Latent"]),
        ],
        [mean2, mean2],
        c="r",
    ...
    ax_test[2].plot(
        [mean1, mean1],
        [
            mean2 - 2 * np.sqrt(ret2["Var_Latent"]),
            mean2 + 2 * np.sqrt(ret2["Var_Latent"]),
        ],
        c="r",
    )
    ...
    ax_test[2].scatter(
        data_test["Y1"], data_test["Y2"], c="k", marker="o", label="Simulation"
    )
    ax_test[2].legend()
...
f.savefig(WORK_DIR / "multi_output_kernel_test_f.png")
fig_test.savefig(WORK_DIR / "multi_output_kernel_test_fig_test.png")
