...
class BaseEmulator:
    ...
    def test(self):
        ...
        if not self.training_complete:
            ...
            self.mse = np.linalg.norm( self.y_test.flatten() - self.y_pred_test, 
                                       ord=2 )
    ...
    def plot_predictions(self):
        ...
        predictions_vs_true['error'] = predictions_vs_true['high'] \
                                       - predictions_vs_true['low']
    
        # Classify data into success and failures
        test_success \
            = predictions_vs_true[ (    predictions_vs_true['low' ]   \
                                     <= predictions_vs_true['true'] ) \
                                  &(    predictions_vs_true['high']   \
                                     >= predictions_vs_true['true'] ) ].copy()
        test_success.rename( columns = { 'value': 'predicted (correct)',
                                         'true' : 'true value' }, inplace=True )
        ...
        test_failure \
            = predictions_vs_true[ (    predictions_vs_true['low' ]   \
                                  ...
                                  |(    predictions_vs_true['high']   \
                                      < predictions_vs_true['true'] ) ].copy()
        test_failure.rename( columns = { 'value': 'predicted (failed)',
                                         'true' : 'true value' }, inplace=True )
