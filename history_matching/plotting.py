...
def plot_errors(train, test, Ycol, desired_result):
    ...
    for data, ax, label in zip( [train_impl, test_impl], [ax1,ax2], ['Train', 'Test']):
        if True in data.index:
            ax.scatter(x=data.loc[data.index==True, Ycol], y=data.loc[data.index==True, 'Z_Noisy'], marker='x', facecolor='r', lw=1, alpha=0.5, s=25, label='Implausible')
        if False in data.index:
            ax.scatter(x=data.loc[data.index==False, Ycol], y=data.loc[data.index==False, 'Z_Noisy'], marker='.', facecolor='k', lw=1, alpha=0.5, s=25, label='Not Implausible')
...
def plot_errors_old(train, test, Ycol, desired_result):
    ...
    ax.errorbar(x=test_impl.loc[test_impl.index==False, Ycol], y=test_impl.loc[test_impl.index==False, 'Mean_Estimate'], yerr=2*np.sqrt(test_impl.loc[test_impl.index==False, 'Var_Err_Predictive']), fmt='o', ms=3, c='m', lw=0.5)
    ...
    ax.scatter(x=test_impl.loc[test_impl.index==True, Ycol], y=test_impl.loc[test_impl.index==True, 'Z_Noisy'], facecolor='m', marker='.', lw=1, alpha=0.5, s=50, edgecolors='r')
    ax.scatter(x=test_impl.loc[test_impl.index==False, Ycol], y=test_impl.loc[test_impl.index==False, 'Z_Noisy'], facecolor='m', marker='.', lw=1, alpha=0.5, s=50, edgecolors='k')
    ax.set_xlabel(Ycol)
    ax.set_ylabel('Z-Score')
    ax.margins(x=0,y=0.05)
    xlim = ax.get_xlim()
        ...
        for col in range(len(Xcols)):
            if col > row:
                ...
                for true_or_false, ec in zip([False, True], ['k', 'r']):
                    ...
                    s = scaled.loc[ td.index == true_or_false ]
