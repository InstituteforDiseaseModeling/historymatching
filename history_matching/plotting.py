    ...
    for row in range(D):
        for col in range(D):
            if col > row:
                ...
                #x = data[ Xcols[row] ]; y = data[ Xcols[col] ] #TODO(dklein): Do we need this?
    ...
    for _ in range(iteration+2):
        ...
        for row in range(D):
            for col in range(D):
                if col > row:
                    gs = gridspec.GridSpec(D-1, D-1)
                    _ = fig.add_subplot(gs[col-1, row])
...
def joint_plot(data, data_mean, Ycol, desired_result, log_x=False):
    fig, _ = plt.subplots(nrows=1, ncols=1, figsize=(16, 10), dpi=300)
        ...
        for col in range(len(Xcols)):
            if col > row:
                ...
                for true_or_false, ec in zip([False, True], ['k', 'r']):
                    ...
                    #TODO(dklein): Should we keep this?
