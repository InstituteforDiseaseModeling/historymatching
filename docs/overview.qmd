...
History matching is an iterative calibration technique that progressively rules out regions of parameter space that are [*implausible*](glossary.qmd#implausibility) — i.e., where simulated outputs are inconsistent with observations. Rather than finding a single "best fit," it maps out the whole region of parameter space that is not inconsistent with the data — a region you can then sample from. That surviving region is called the [**NROY**](glossary.qmd#nroy) ("Not Ruled Out Yet") region, and it is the central output of the method.

Each iteration is called a [**wave**](glossary.qmd#wave).
...
3. **Select [features](glossary.qmd#feature-output)** — choose which model outputs to emulate (ranked by [mean-squared z-score](glossary.qmd#mean-sq-z), or specified manually)
4. **Train [emulators](glossary.qmd#emulator)** — build fast statistical surrogates (Bayes linear by default; Gaussian Process Regression and others available) of the simulation
5. **Find non-implausible points** — generate candidate points and keep those consistent with the observations across *all* emulators trained so far (found via [LHS](glossary.qmd#lhs) or [ray sampling](glossary.qmd#ray-resample)); these seed the next iteration
...
- **Multiple emulators**: [Bayes linear](glossary.qmd#bayes-linear) (the default — nearly GPR-quality at a fraction of the cost), [linear](glossary.qmd#linear), [GLM](glossary.qmd#glm), and [Gaussian Process Regression](glossary.qmd#gpr) (GPflow-based with [ARD](glossary.qmd#ard) kernels)
