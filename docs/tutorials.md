...
- [NROY sampling methods](tutorials/06_nroy_sampling_methods.ipynb) — Comparison of sampling strategies for exploring the non-implausible space
...
- [Trajectory selection](tutorials/05_trajectory_selection.ipynb) — Draw plausible `(θ, seed)` pairs (here `θ = (β, γ)`) for stochastic simulation using importance resampling
