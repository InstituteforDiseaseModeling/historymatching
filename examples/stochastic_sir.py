...
RUNS_PER_WAVE = 50
DEFAULT_FIG_DIR = Path(__file__).parent.absolute() / "figures"


def main(save_plots: bool = False, figdir: Path = DEFAULT_FIG_DIR) -> None:

    save_plots = True   # clorton

    figdir.mkdir() if save_plots and not figdir.exists() else None
    ...
    parameter_samples = hm2.sampling.latin_hypercube(
        param_info, samples=RUNS_PER_WAVE, random_state=123456
    )
    ...
    if save_plots:
        ...
        p["all"].fig.savefig(figdir / f"01_pairwise_parameter_samples.png")
    ...
    sim_replicates = run_replicates(
        ...
        replicates=4,
        processes=1,    # None == use all cores are to make multiple runs in parallel
    )

    if save_plots:
        # Just for fun, let's visualize some SIR trajectories and observations / target data
        p = plot_runs_time_series(
            sim_replicates, samples=20, real_observations=real_observations
        )
        p.save(figdir / "02_replicates.png")
    ...
    matched = match_sim_outputs_to_observations(
        sim_replicates, real_observations, processes=None
    )
    ...
    params, y, stdev = get_single_obs_data_for_emulators(
        parameter_samples, matched, observation_id=0
    )

    # Train emulator
    # Note 1: https://scikit-learn.org/stable/modules/preprocessing.html
    # Note 2: https://github.com/ppgaluzio/MOBOpt/issues/13
    ...
    p3 = emulator.glm.plot_deviance_residuals()
    p4 = emulator.glm.plot_QQ()
    p5 = emulator.plot_data()["all"]
    p6 = emulator.plot_emulated_vs_predicted()
    if save_plots:
        for p, name in [
            (p1, "03_fitted_vs_observed"),
            (p2, "04_pearson_residuals"),
            (p3, "05_deviance_residuals"),
            (p4, "06_qq"),
            (p5, "07_all_data"),
            (p6, "08_emulated_vs_predicted")]:
            p.fig.savefig(figdir / f"{name}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--plots", default=False, action="store_true", help="Save plots to disk")
    parser.add_argument("-s", "--seed", type=np.uint32, default=20220816, help="PRNG seed [20220816]")
    parser.add_argument("-f", "--figdir", type=str, default=DEFAULT_FIG_DIR, help=f"directory for saved plots (figures) [{DEFAULT_FIG_DIR}]")

    args = parser.parse_args()

    np.random.seed(args.seed)

    main(save_plots=args.plots, figdir=args.figdir)
