...
def trajectory(params: np.array) -> Tuple[np.array, np.array, np.array]:
    ...
    y0 = np.array([POPULATION - params[2], params[2], 0.])  # S, I, R
    t0 = 0.
    ...
    tmax = float(DURATION)
    dt = 1.
    sus = np.zeros(int(tmax) + 1); sus[0] = y0[0]
    inf = np.zeros(int(tmax) + 1); inf[0] = y0[1]
    rec = np.zeros(int(tmax) + 1); rec[0] = y0[2]
...
def main():
    ...
    for iteration in sorted(set(situation.simulator_results.iteration)):
        ...
        for row in points.itertuples():
            ...
            plt.subplot(1, 3, 1); plt.plot(s, linewidth=1)
            plt.subplot(1, 3, 2); plt.plot(i, linewidth=1)
            c = np.zeros_like(i)
            c[1:] = i[1:] - i[:-1]
            plt.subplot(1, 3, 3); plt.plot(c, linewidth=1)
        s, i, r = trajectory(np.array([TARGET_BETA, TARGET_GAMMA, TARGET_INITIAL]))
        plt.subplot(1, 3, 1); plt.plot(s, color="green", linewidth=2)
        plt.subplot(1, 3, 2); plt.plot(i, color="red", linewidth=2)
        c = np.zeros_like(i)
        c[1:] = i[1:] - i[:-1]
        plt.subplot(1, 3, 3); plt.plot(c, color="blue", linewidth=2)

        plt.subplot(1, 3, 1); plt.plot(90, s[90], "o", color="green")
        plt.subplot(1, 3, 2); plt.plot(14, i[14], "o", color="red")
        plt.subplot(1, 3, 3); plt.plot(21, i[21] - i[20], "o", color="blue")
