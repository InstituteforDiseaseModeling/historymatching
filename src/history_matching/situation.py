...
class Situation:
    ...
    def validate_iteration(iteration: int) -> None:
        ...
        if not isinstance(iteration, (int, float, np.number)):
            raise TypeError(f"Situation iteration, {iteration}, should be numeric, not '{type(iteration)}'")
        if int(iteration) != iteration:
            raise ValueError(f"Situation iteration should be an integer value, not {iteration}")
        if iteration < 0:
    ...
    def validate_sample_points(sample_points: pd.DataFrame, parameter_space: pd.DataFrame) -> None:
        ...
        if not isinstance(sample_points, pd.DataFrame):
            raise TypeError(f"Situation sample points should be Pandas DataFrame, not '{type(sample_points)}'")
        ...
        if not all(column in sample_points.columns for column in required_columns):
            raise RuntimeError(f"Situation sample points must contain the columns {required_columns}. Found {sample_points.columns}.")
        if len(sample_points) == 0:
            raise RuntimeError("Situation sample points must specify at least one point in parameter space. Found none.")
        valid = True
        msg = ""
    ...
    @staticmethod
    def validate_emulator_bank(emulator_bank: Dict[int, Dict[str, BaseEmulator]], observations: pd.DataFrame) -> None:
        """
        ...
        if not isinstance(emulator_bank, dict):
            raise TypeError(f"Situation enumlator bank should be dictionary, not '{type(emulator_bank)}'")

        # emulator_bank must be a dictionary mapping **int**:dict
        if not all(isinstance(k, (int, np.number)) for k in emulator_bank.keys()):
            raise TypeError(f"Situation emulator bank keys should be integer values. Found {list(emulator_bank.keys())}")

        # emulator_bank must be a dictionary mapping int:**dict**
        if not all(isinstance(v, dict) for v in emulator_bank.values()):
            raise TypeError(f"Situation emulator bank values must be dictionaries. Found {[type(v) for v in emulator_bank.values()]}")

        # keys must be >= 0
        if not all(k >= 0 for k in emulator_bank.keys()):
        ...
        for iteration, emulators in emulator_bank.items():
            # Values must be dictionaries mapping **str**:BaseEmulator
            if not all(isinstance(k, str) for k in emulators.keys()):
                raise TypeError(f"Situation emulator bank values should be dictionaries with string keys. Found {[type(k) for k in emulators.keys()]}")

            # Values must be dictionaries mapping str:**BaseEmulator**
            if not all(isinstance(v, BaseEmulator) for v in emulators.values()):
                raise TypeError(f"Situation emulator bank values should be dictionaries mapping features to emulators. Found {[type(v) for v in emulators.values()]}")
            ...
            if not all(key in features for key in emulators.keys()):
                raise ValueError(f"Found 'feature' in emulators dictionary ({emulators.keys()}, iteration {iteration}) which does not map to observation features ({features}).")
