...
from history_matching import Config, latin_hypercube_sampler as lhs
...
#    num_trials = 0
    ...
    stats = {k: {'cut_implausible': 0, 'newly_implausible': 0, 'num': 0} for k in emulator_bank}
...
#        proposed_nSamples = num_desired_candidates if stats['num_candidates'] == 0 else \
#            int(round(1.25 * (num_desired_candidates - stats['num_plausible_candidates']) / (1 - stats['num_plausible_candidates'] / stats['num_candidates'])))
#        nSamples = min(max_nSamples, proposed_nSamples)

#        new_samples = lhs(parameter_space, nSamples)
#        new_candidates = pd.DataFrame(new_samples)

#        plausibility = test_plausibility(new_candidates, emulator_bank, config)
#        new_candidates
        pass
...
#                 emulated_values = emulator.evaluate(plausible_candidates)
#                 # Mean_Estimate = Yglm (from GLM) + Mean (from GPR)
#                 # desired_result ¿target from feature selection? (TBD)
#                 # Var_Predictive (from GPR)
#                 # desired_result_var ¿observation variance? (TBD)
#                 # discrepancy_var ¿? (TBD)
#                 implausibility = abs(plausible_candidates['Mean_Estimate'] - self.hm_params[cut]['desired_result']) /
#                                  np.sqrt(plausible_candidates['Var_Predictive'] + self.hm_params[cut]['desired_result_var'] + self.hm_params[cut]['discrepancy_var'])
#                 # implausibility_threshold - ¿user configured?
