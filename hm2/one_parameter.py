...
def main():
        ...
        while len(points) < 11:
            ...
            for iteration in reversed(range(iteration+1)):
                ...
                for feature, emulator in emulators.items():
                    ...
                    difference = np.abs(prediction.value - np.float64(observations["means"][feature]))
    ...
    print(f"Correct intercept, based on noisy sampling = {np.float64(observations['means']['x5']) - (SLOPE*5)}")
    print(observations)
