...
class State:
    ...
    def validate_parameter_space(parameter_space: pd.DataFrame) -> None:
        ...
        if not all(
            ...
            raise RuntimeError(
                f"State parameter space must contain the columns 'parameter', 'minimum', 'maximum'. Found {parameter_space.columns}."
            )
    ...
    def validate_observations(observations: pd.DataFrame) -> None:
        ...
        if not "statistic" in observations.columns:
            raise RuntimeError(f"State observations must have 'statistic' column. Found {observations.columns}")

        if len(observations.columns) <= 1:
            raise RuntimeError(
                f"State observations must have at least one feature column. Found {observations.columns}"
            )

        if len(observations[observations.statistic == "mean"]) != 1:
            raise RuntimeError(f"State observations must have one row for means. Found {observations.statistic}")

        if len(observations[observations.statistic == "variance"]) != 1:
            raise RuntimeError(f"State observations must have one row for variances. Found {observations.statistic}")
    ...
    def validate_emulator_bank(
        ...
        if not all(map(lambda k: isinstance(k, (int, np.number)), emulator_bank.keys())):
            raise TypeError(f"State emulator bank keys should be integer values. Found {list(emulator_bank.keys())}")

        # emulator_bank must be a dictionary mapping int:**dict**
        if not all(map(lambda v: isinstance(v, dict), emulator_bank.values())):
            raise TypeError(f"State emulator bank values must be dictionaries. Found {list(map(lambda v: type(v), emulator_bank.values()))}")

        # keys must be >= 0
        if not all(map(lambda k: k >= 0, emulator_bank.keys())):
            raise ValueError(f"State emulator bank keys must be >= 0. Found {list(emulator_bank.keys())}")
        ...
        for iteration, emulators in emulator_bank.items():
            ...
            if not all(map(lambda k: isinstance(k, str), emulators.keys())):
                raise TypeError(f"State emulator bank values should be dictionaries with string keys. Found {list(map(lambda k: type(k), emulators.keys()))}")

            # Values must be dictionaries mapping str:**BaseEmulator**
            if not all(map(lambda v: isinstance(v, BaseEmulator), emulators.values())):
                raise TypeError(f"State emulator bank values should be dictionaries mapping features to emulators. Found {list(map(lambda v: type(v), emulators.values()))}")
