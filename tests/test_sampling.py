...
class TestSamplingStrategy:
    ...
    def test_subclass_must_implement_abstract_methods(self):
        ...
        class IncompleteSampler(hm.SamplingStrategy):
            pass
...
class TestLatinHypercubeSampling:
    ...
    def test_initialization_default_params(self):
        ...
        sampler = hm.LatinHypercubeSampling()
        assert sampler.criterion == 'maximin'
        assert sampler.iterations == 5
    
    def test_initialization_custom_params(self):
        """Test initialization with custom parameters."""
        sampler = hm.LatinHypercubeSampling(criterion='center', iterations=10)
    ...
    def test_get_strategy_name(self):
        ...
        sampler = hm.LatinHypercubeSampling(criterion='center')
...
class TestGridSampling:
    ...
    def test_initialization_default_params(self):
        """Test initialization with default parameters."""
        sampler = hm.GridSampling()
        assert sampler.samples_per_dimension is None
    
    def test_initialization_custom_params(self):
        """Test initialization with custom parameters."""
        sampler = hm.GridSampling(samples_per_dimension=5)
    ...
    def test_parameter_validation_invalid_samples_per_dimension(self):
        ...
        with pytest.raises(ValueError, match="samples_per_dimension must be >= 1"):
            hm.GridSampling(samples_per_dimension=0)
...
class TestRandomSampling:
    ...
    def test_initialization(self):
        """Test initialization."""
        sampler = hm.RandomSampling()
        # No specific attributes to test
        assert isinstance(sampler, hm.RandomSampling)
...
class TestSamplingStrategyFactory:
    ...
    def test_create_lhs_strategy(self):
        ...
        assert isinstance(strategy, hm.LatinHypercubeSampling)
    ...
    def test_create_lhs_strategy_with_params(self):
        ...
        strategy = hm.SamplingStrategyFactory.create('lhs', criterion='center', iterations=10)
        assert isinstance(strategy, hm.LatinHypercubeSampling)
    ...
    def test_create_grid_strategy(self):
        ...
        strategy = hm.SamplingStrategyFactory.create('grid')
        assert isinstance(strategy, hm.GridSampling)
    
    def test_create_random_strategy(self):
        """Test creating Random Sampling strategy."""
        strategy = hm.SamplingStrategyFactory.create('random')
        assert isinstance(strategy, hm.RandomSampling)
    
    def test_create_strategy_case_insensitive(self):
        """Test that strategy creation is case insensitive."""
        strategy1 = hm.SamplingStrategyFactory.create('LHS')
        strategy2 = hm.SamplingStrategyFactory.create('Latin_Hypercube')
        assert isinstance(strategy1, hm.LatinHypercubeSampling)
        assert isinstance(strategy2, hm.LatinHypercubeSampling)
    
    def test_create_unknown_strategy(self):
        """Test creating unknown strategy raises error."""
        with pytest.raises(ValueError, match="Unknown sampling strategy"):
            hm.SamplingStrategyFactory.create('unknown_strategy')
    
    def test_register_custom_strategy(self):
        """Test registering a custom strategy."""
        
        class CustomSampler(hm.SamplingStrategy):
        ...
        hm.SamplingStrategyFactory.register_strategy('custom', CustomSampler)
        
        # Test that we can create it
        strategy = hm.SamplingStrategyFactory.create('custom')
        assert isinstance(strategy, CustomSampler)
        
        # Clean up
        del hm.SamplingStrategyFactory._strategies['custom']
    ...
    def test_register_invalid_strategy_class(self):
        ...
        with pytest.raises(TypeError, match="must be a subclass of SamplingStrategy"):
            hm.SamplingStrategyFactory.register_strategy('invalid', NotASampler)
    
    def test_available_strategies(self):
        """Test getting available strategies."""
        strategies = hm.SamplingStrategyFactory.available_strategies()
        expected = ['lhs', 'latin_hypercube', 'grid', 'random', 'uniform']
        assert set(strategies) == set(expected)
    
    def test_get_strategy_info(self):
        """Test getting strategy information."""
        info = hm.SamplingStrategyFactory.get_strategy_info('lhs')
    ...
    def test_get_strategy_info_unknown(self):
        ...
        with pytest.raises(ValueError, match="Unknown strategy"):
            hm.SamplingStrategyFactory.get_strategy_info('unknown')
...
class TestSamplingIntegration:
    ...
    def test_all_strategies_produce_valid_samples(self, parameter_space):
        ...
        for strategy_name in strategies:
            strategy = hm.SamplingStrategyFactory.create(strategy_name)
