...
class SamplingTest(unittest.TestCase):
    def test_missing_name(self):
        param_info = pd.DataFrame(
            {"name2": ["Beta", "Gamma"], "min": [1e-6, 1e-6], "max": [0.01, 0.5]}
        )
    ...
    def test_missing_min(self):
        param_info = pd.DataFrame(
            {"name": ["Beta", "Gamma"], "min2": [1e-6, 1e-6], "max": [0.01, 0.5]}
        )
        with self.assertRaises(HMMissingColumn) as cm:
            latin_hypercube(param_info, 30)
    ...
    def test_missing_max(self):
        param_info = pd.DataFrame(
            {"name": ["Beta", "Gamma"], "min": [1e-6, 1e-6], "max2": [0.01, 0.5]}
        )
        with self.assertRaises(HMMissingColumn) as cm:
            latin_hypercube(param_info, 30)
    ...
    def test_max_smaller_than_min(self):
        param_info = pd.DataFrame(
            {"name": ["Beta", "Gamma"], "min": [3, 4], "max": [100, -100]}
        )
        self.assertRaises(HMMaxLessThanMin, latin_hypercube, param_info, 30)

    def test_scaling(self):
        param_info = pd.DataFrame(
            {"name": ["Beta", "Gamma"], "min": [-100, 10], "max": [100, 100]}
        )
...
class SampleWithinTest(unittest.TestCase):
    ...
    def test_empty(self):
        ...
        self.assertRaises(
            HMParameterSamplesEmpty, latin_hypercube_within, param_info, 30
        )
...
class TestVoume(unittest.TestCase):
    def test_point_volume(self):
        param_info = pd.DataFrame(
            {"name": ["Beta", "Gamma", "Mu"], "min": [-2, 4, 3], "max": [2, 8, 6]}
        )
    ...
    def test_volume(self):
        ...
        self.assertAlmostEqual(
            get_size_of_parameter_space(cube), 4.0 * 4.0 * 3.0, places=2
        )

    def test_percent_change(self):
        pc = percent_change_vol(vol_old=30, vol_new=24)
        self.assertAlmostEqual(pc, (24 - 30) / 30 * 100.0)
