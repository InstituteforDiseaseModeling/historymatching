...
class UtilityTests(unittest.TestCase):
    def test_mean_and_variance_for_observations(self):
        ...
        assert np.float64(mean_and_variance.means["height"]) == heights.mean()
        assert np.float64(mean_and_variance.means["weight"]) == weights.mean()
        assert np.float64(mean_and_variance.variances["height"]) == heights.var(ddof=1)  # Use N-1 for variance
        assert np.float64(mean_and_variance.variances["weight"]) == weights.var(ddof=1)  # Use N-1 for variance
