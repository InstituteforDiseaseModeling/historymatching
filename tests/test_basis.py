...
class TestIdentityBasis(unittest.TestCase):
    ...
    def test_bad_scale(self):
        self.assertRaises(
            AssertionError, hm2.basis.IdentityBasis, intercept=True, scale="bad"
        )
...
class TestPolynomialBasis(unittest.TestCase):
    def test_bad_intercept(self):
        self.assertRaises(
            AssertionError, hm2.basis.PolynomialBasis, degree=2, intercept="bad"
        )

    def test_bad_scale(self):
        self.assertRaises(
            AssertionError,
            hm2.basis.PolynomialBasis,
            degree=2,
            intercept=True,
            scale="bad",
        )
    ...
    def test_degree_bad(self):
        self.assertRaises(
            AssertionError, hm2.basis.PolynomialBasis, degree="bad", intercept=True
        )

    def test_degree_good_number(self):
        self.assertRaises(
            AssertionError, hm2.basis.PolynomialBasis, degree=-1, intercept=True
        )
